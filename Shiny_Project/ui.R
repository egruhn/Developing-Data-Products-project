        ...
        navbarPage(
                ...
                tabPanel(
                        ...
                        sidebarPanel(
                                ...
                                sliderInput("cyl", "Cylinders", min = round(minCylinders), max = round(maxCylinders), value = defaultCar$cyl, step = 1),
                                bsTooltip(id = "cyl", title = "Number of cylinders in the engine", placement = "right", options = list(container = "body")),

                                sliderInput("disp", "Displacement", min = round(minDisp), max = round(maxDisp), value = defaultCar$disp, step = 1),
                                bsTooltip(id = "disp", title = "Engine displacement (in cu.in.)", placement = "right", options = list(container = "body")),

                                sliderInput("hp", "Horsepower", min = round(minHp), max = round(maxHp), value = defaultCar$hp, step = 10),
                                bsTooltip(id = "hp", title = "Engine Horsepower", placement = "right", options = list(container = "body")),

                                sliderInput("drat", "Rear axle ratio", min = round(minDrat), max = round(maxDrat), value = defaultCar$drat, step = 0.01),
                                bsTooltip(id = "drat", title = "Rear axle ratio", placement = "right", options = list(container = "body")),

                                sliderInput("wt", "Weight", min = round(minWt), max = round(maxWt), value = defaultCar$wt, step = 1),
                                bsTooltip(id = "wt", title = "Weight in 1000 lbs", placement = "right", options = list(container = "body")),

                                sliderInput("qsec", "1/4 mile time", min = round(minQsec), max = round(maxQsec), value = defaultCar$qsec, step = 0.1),
                        ...
                        mainPanel(
                                ...
                                p("Your expected gas consumption will be: "),
                ...
                tabPanel(
                        ...
                        p("User can freely adjust the slider values in order to simulate the parameters of an hypothetical car and be able to predict its MPG consumption."),
                ...
                tabPanel(
                        ...
                        h3("Developing Data Products Project - Shiny Application and Reproducible Pitch"),

                        h3("Author: Erich F Gruhn - May 12, 2018"),
                        ...
                        p("Source code of this application is available at",
                          a(href = "https://github.com/egruhn/Developing-Data-Products-project",
                            "https://github.com/egruhn/Developing-Data-Products-project")
                        )
